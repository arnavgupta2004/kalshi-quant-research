...
| | Question | Answer |
|---|---|---|
| **A** | How often are structural probability constraints violated? | Rarely: 12.9 [5.9, 22.5] standing violations per 1,000 relation-hours (fresh recordings); every proven, lattice or declared relation held in **28,548** real settled instances with **0** violations. |
| **B** | What fraction of detected opportunities are executable after fees and liquidity? | Almost none: funnel of confirmed episodes displayed 24 → liquid 22 → after fees 3 → after slippage 3 → executable 1. Stage 4: 327 displayed violations, **0** survive fees (54 would fee-free). |
| **C** | How fast does apparent edge disappear? | Standing edges last seconds (median lifetime 9.6 to 12.0 s); most that vanish move or are pulled rather than being taken (hypothesis C2 failed: 33% consumed). |
| **D** | How does latency affect executable arbitrage? | 1–250 ms cannot be resolved from snapshots seconds apart (reported as brackets); through the engine, partial (leg-risk) bundles at 250 ms and 3 s: 250 ms: 0/105, 3000 ms: 28/106; but with real fees there is almost no P&L to lose (max over latencies of scored full-bundle P&L = $2.51). |
...
-1.42 [-1.85, -1.04]¢), under every fill model and latency (10 of 10 cells negative) and every risk
...
| 3 | contract semantics, relations with evidence levels | 0 violations of 28,548 proven/lattice/declared instances in 14,383 settled events; empirical-level relations violated 0.11%, unverified 4.6% |
| 4 | same-event arbitrage detector, fee model | 327 displayed violations in 44 minutes, 0 after fees |
| 5 | event-driven replay engine (latency, queue models, look-ahead audit) | a passive strategy's P&L spans $74 across queue assumptions and is not profitable under the most generous (-$0.27) |
| 6 | arbitrage research: episodes, lifetimes, latency, categories | 11 of 15 pre-registered hypotheses held; the four that failed are reported |
| 7–8 | probability models; calibration on a sealed holdout | no model beats the market price (log loss 0.3518 vs 0.3526 for the microstructure model, 0.681 for a constant); the price is calibrated |
| 9 | bounded-support market maker, binary-contract inventory risk | net -$426; adverse selection |
| 10 | adaptive maker, signal study, ablation ladder | see §6 |
| 11 | paper trading on live data | on three live sessions the orders, fills and equity were identical to a backtest of the session's own tape; strategy reaction 19 ms median, 112 ms p99 |
...
| `relations.duckdb` | 14,383 settled events, 142,709 markets | relation semantics and their real-outcome audit |
...
**Results** (fresh recordings; 24 confirmed episodes, 50 sightings):

* **A — frequency.** 12.9 [5.9, 22.5] confirmed per 1,000 relation-hours (26.8 [14.5, 43.9] counting every
  sighting). Not stable across windows: the development data gave 31.1 [12.9, 56.4], because one live
...
* **B — executability.** Fees, then size, remove the edge; the structure is right. Whenever a relation
  settled, the bundle paid at least what it promised (`0 shortfalls in 133 settled tradable episodes (0 PROVEN)`). The funnel, confirmed episodes at
  100 contracts:

| stage (confirmed episodes) | surviving | share of displayed |
|---|---|---|
| displayed | 24 | 1.00 [1.00, 1.00] |
| liquid | 22 | 0.92 [0.81, 1.00] |
| after fees | 3 | 0.12 [0.00, 0.28] |
| after slippage | 3 | 0.12 [0.00, 0.28] |
| executable | 1 | 0.04 [0.00, 0.14] |

* **C — decay.** Median lifetime 9.6 to 12.0 s. What ended the confirmed episodes: 33% consumed (trade-tape attribution is only as sharp as the polling interval).
* **D — latency.** Partial (leg-risk) bundles: 250 ms: 0/105, 3000 ms: 28/106. Scored fully hedged bundles: max over latencies of scored full-bundle P&L = $2.51. Latencies below the polling cadence are brackets, not measurements.
* **Categories.** Only Sports has enough events to say anything; every other category is flagged and
  not interpreted.

| category | relation-hours | events | confirmed episodes | per 1,000 rh |
|---|---|---|---|---|
| Climate and Weather | 72 | 5 | 5 | 69.2 [0.0, 180.0] |
| Commodities | 56 | 1 | 0 | 0.0 |
| Crypto | 593 | 6 | 0 | 0.0 [0.0, 0.0] |
| Economics | 40 | 1 | 0 | 0.0 |
| Entertainment | 190 | 8 | 0 | 0.0 [0.0, 0.0] |
| Politics | 81 | 3 | 0 | 0.0 [0.0, 0.0] |
| Science and Technology | 36 | 2 | 0 | 0.0 [0.0, 0.0] |
| Sports | 794 | 128 | 19 | 23.9 [10.9, 40.9] |

**Pre-registered predictions that failed** (11 of 15 held): A3 (8% of 26 under 1 s), A4 (counter-examples: Climate and Weather (5 confirmed, 5 events)), C2
(33% consumed), D1 (width 4 pp at 1 ms, 0 pp at 1 s). They are results, not embarrassments: the chunk-skew artefact that dominated
...
**Limitations.** ~211 observed minutes on one Saturday, 24 confirmed episodes; polling
...
**Results.** On the research period no model beats the market reference (log loss 0.3518; the
microstructure model 0.3526; a constant 0.681). On the sealed holdout (1,776
instances, 731 events): slope 0.97 [0.86, 1.11], YES-minus-price -0.012 [-0.038, 0.014]; 0 of 23 subgroup gaps have an interval excluding 0. All
17 of 17 criteria held. Order-book information does not help the outcome forecast: microprice minus mid = +0.0157 (681 instances).
...
**Results** (fresh recordings, 2,410 fills, 81 events):

* net **-$426**: spread captured -$21, inventory contribution -$392; fees are
  a small part. 30 s markout **-1.42 [-1.85, -1.04]¢**; settled P&L per fill **-0.177 [-0.229, -0.130] $**.
* negative under every fill model and latency (10 of 10 cells; -$426 to -$495); in
  0 of 9 of the risk-aversion × spread grid cells is settled P&L per fill significantly above zero.

Each design element removed in turn:

| variant | net | contracts | net / contract | mean abs inventory | max drawdown |
|---|---|---|---|---|---|
| A naive join (best bid, no model) | -$1,101 | 127,254 | -0.9¢ | 28 | $858 |
| B fixed width, no skew, no limits | -$623 | 25,658 | -2.4¢ | 52 | $566 |
| C fixed width, no skew, limits + kill | -$490 | 20,522 | -2.4¢ | 23 | $395 |
| D skew, no limits | -$744 | 40,082 | -1.9¢ | 6 | $600 |
| E skew, limits, no kill-switch | -$744 | 40,082 | -1.9¢ | 6 | $600 |
| F baseline: skew + limits + kill | -$426 | 22,415 | -1.9¢ | 6 | $300 |

Skew cuts mean inventory (6 vs 52 contracts) without making money; the kill-switch caps the
drawdown ($300 vs $600) without creating edge.

**By category:** 1,805 of 2,410 fills are Sports (markout -1.58 [-2.09, -1.13]¢); the other
categories have too few events to say anything.

| category | fills | events | 30 s markout (¢) | settled P&L / fill ($) |
|---|---|---|---|---|
| Climate and Weather | 314 | 5 | -1.35 [-2.79, 0.22] | n/a |
| Commodities | 14 | 1 | 0.61 | n/a |
| Crypto | 198 | 5 | -0.87 [-1.49, 1.51] | -0.144 |
| Economics | 3 | 1 | 1.17 | n/a |
| Entertainment | 46 | 7 | 0.57 [-0.37, 2.71] | n/a |
| Politics | 23 | 1 | -0.09 | -0.016 |
| Science and Technology | 7 | 1 | -2.00 | n/a |
| Sports | 1805 | 60 | -1.58 [-2.09, -1.13] | -0.181 [-0.239, -0.129] |

**Experiment G — time to resolution.** The remaining standard deviation of the inventory a maker is left
holding, |q|√(p(1−p)), is flat across time-to-resolution ($2.17-$2.52): risk depends on the state
(the price), not on a (T−t) clock. Adverse selection is not worse near resolution (-1.09 [-1.65, -0.68]¢ (6 events)
...
skill 5.3% [3.4, 7.6] against "the mid does not move". Order-book imbalance and the microprice fail
out of sample (validation skill -0.57% [-1.58, 0.17]), as do flow and momentum; adding them to the staleness
model lowers skill. The *size* of the next move is forecastable (skill 11.2% [8.2, 14.2]); its direction
is not. And fill toxicity barely depends on that size: the 30 s markout of baseline fills by forecast-scale
tercile is calm -1.63¢; middle -1.66¢; volatile -2.15¢, around a constant of -1.66¢ (slope -0.13¢ per ¢).
...
| variant (validation databases) | net | contracts | settled P&L / contract (¢) | vs baseline (¢) |
|---|---|---|---|---|
| 0 baseline (Stage 9) | -$317 | 13,360 | -3.2 [-4.4, -2.2] | - |
| 1 + re-price on prints (no model) | -$333 | 13,538 | -3.2 [-4.4, -2.2] | -0.07 [-0.55, 0.50] |
| 2 + fair value: staleness correction | -$314 | 13,731 | -2.8 [-3.8, -1.9] | 0.34 [-0.23, 0.94] |
| 3 + fair value: staleness + order book, flow, momentum | -$324 | 14,060 | -2.6 [-3.7, -1.7] | 0.54 [-0.08, 1.17] |
| 4 + widening by expected move | -$271 | 10,443 | -3.4 [-4.7, -2.0] | -0.26 [-1.27, 0.80] |
| 5 + size by expected move | -$118 | 4,377 | -4.6 [-7.1, -2.3] | -1.40 [-3.57, 0.40] |
| 6 + time-to-resolution skew (= FULL) | -$131 | 4,460 | -4.8 [-7.1, -2.6] | -1.61 [-3.33, 0.02] |
| full - fair value | -$146 | 5,052 | -4.5 [-6.9, -2.3] | -1.28 [-3.08, 0.26] |
| full - widening | -$148 | 5,737 | -3.9 [-6.0, -1.9] | -0.69 [-2.06, 0.72] |
| full - size | -$276 | 10,704 | -3.2 [-4.5, -2.0] | 0.01 [-0.96, 1.10] |
| full - time skew | -$118 | 4,377 | -4.6 [-7.1, -2.3] | -1.40 [-3.57, 0.40] |

Contrasts between rungs (¢ per contract): staleness fair value over re-pricing alone
**0.41 [0.10, 0.79]**; book, flow and momentum on top of staleness 0.19 [-0.14, 0.61]; widening -0.61 [-1.59, 0.26]; sizing
-1.14 [-3.10, 0.23]; time skew -0.21 [-0.76, 0.57].
...
2. **Nothing else improves fill quality.** Genuine microstructure features add nothing significant;
   ...
   -4.3, -4.8, -5.0, -6.8); the time skew changes nothing.
3. **Sizing cuts the loss by trading less, not by trading better**: net -$317 → -$131
   with 67% fewer contracts, at a per-contract point estimate that is worse. A losing
   strategy that trades less loses less.
4. **The adaptive maker still loses under every fill model and latency** (10 of 10 cells), and
...
backtest engine: on three live sessions (1,808 simulated orders) the orders, fills
and equity were identical, with a strategy reaction of 19 ms median, 112 ms p99. The WebSocket path could not
...
|---|---|---|---|
| Stage 6 arbitrage (fresh recordings) | 11 of 15 | 0 | A3, A4, C2, D1 |
